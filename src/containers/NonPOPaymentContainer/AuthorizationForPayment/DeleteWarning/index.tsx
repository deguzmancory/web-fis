...
import { useDeleteAuthorization } from 'src/queries';
...
const DeleteAuthorizationPaymentWarning: FC<Props> = ({
  id,
  onHideDialog,
  onSetFormData,
  onDelete,
}) => {
  const { deleteAuthorizationPayment, isLoading: isLoadingAuthorizationPayment } =
  ...
  const handleDelete = () => {
    deleteAuthorizationPayment({ id: id });
    if (onDelete) {
      onDelete();
    }
  };
    ...
    <Box>
      ...
      <Stack direction={'row'} justifyContent="flex-end" mt={3}>
        <Button
          ...
          disabled={isLoadingAuthorizationPayment}
        >
          Cancel
        </Button>
        <Button
          isLoading={isLoadingAuthorizationPayment}
          disabled={isLoadingAuthorizationPayment}
...
export default connect(mapStateToProps, mapDispatchToProps)(DeleteAuthorizationPaymentWarning);
