import { EmployeeStatus, UHBUNumber, UHPRNumber } from 'src/queries';
...
export const EmployeeStatusOptions = [
  { label: 'RCUH', value: EmployeeStatus.RCUH },
  { label: 'UH', value: EmployeeStatus.UH },
  { label: 'Emeritus', value: EmployeeStatus.Emeritus },
  { label: 'Non-comp Appointees', value: EmployeeStatus.NonCompAppointees },
];

export const UHBUNumberOptions = Object.keys(UHBUNumber).reduce(
  (options, key) => [
    ...options,
    {
      label: UHBUNumber[`${key}`],
      value: UHBUNumber[`${key}`],
    },
  ],
  []
);

export const UHPRNumberOptions = Object.keys(UHPRNumber).reduce(
  (options, key) => [
    ...options,
    {
      label: UHPRNumber[`${key}`],
      value: UHPRNumber[`${key}`],
    },
  ],
  []
);
